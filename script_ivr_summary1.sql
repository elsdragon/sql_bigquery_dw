CREATE OR REPLACE TABLE keepcoding.ivr_summary1 AS
...
, next_prev_call
AS (SELECT DISTINCT calls_ivr_id as ivr_id
     , calls_phone_number AS phone_number
     , calls_start_date AS start_date
     , LAG(calls_start_date) OVER (PARTITION BY calls_phone_number ORDER BY calls_ivr_id) AS prev_call
     , LEAD(calls_start_date) OVER (PARTITION BY calls_phone_number ORDER BY calls_ivr_id) AS next_call
     , TIMESTAMP_DIFF(calls_start_date, LAG(calls_start_date) OVER (PARTITION BY calls_phone_number ORDER BY calls_ivr_id), HOUR) AS diff_prev_call
     , TIMESTAMP_DIFF(LEAD(calls_start_date) OVER (PARTITION BY calls_phone_number ORDER BY calls_ivr_id), calls_start_date, HOUR) AS diff_next_call
FROM `keepcooding.keepcoding.ivr_detail`
